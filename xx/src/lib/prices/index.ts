...
async function build(): Promise<PriceSnapshot> {
  ...
  // Crypto: Binance (near real-time) for every spot-listed coin; CoinGecko fills
  // whatever Binance doesn't list (Alpha / non-spot: HYPE, BAS, BLESS, LEO, CRO…).
  // `binanceIds` = cgIds priced by Binance → those get live WS ticks.
