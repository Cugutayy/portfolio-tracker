...
export default function RunsExplorerMap({
    ...
    (acts: CommunityActivity[]) => {
      ...
      for (const act of acts) {
        ...
        try {
          ...
          const coords = decoded.map(
            ([lat, lng]) => [lng, lat] as [number, number]
          );
          features.push({
            type: "Feature",
            properties: { id: act.id },
  ...
  const buildPointsGeoJSON = useCallback((acts: CommunityActivity[]) => {
    const features: GeoJSON.Feature[] = [];
    for (const act of acts) {
      ...
      features.push({
        ...
        geometry: {
          type: "Point",
          coordinates: [act.startLng, act.startLat],
        },
      });
    }
  ...
  // Update route data when activities change
  useEffect(() => {
    ...
    if (routesSrc) routesSrc.setData(buildRoutesGeoJSON(activities));
    if (pointsSrc) pointsSrc.setData(buildPointsGeoJSON(activities));
  }, [activities, buildRoutesGeoJSON, buildPointsGeoJSON]);
  ...
  }, [hoveredId, selectedId, activities]);
  ...
  useEffect(() => {
    ...
    if (map.getLayer("all-routes")) {
      map.setLayoutProperty("all-routes", "visibility", routesVisible ? "visible" : "none");
    }
    if (map.getLayer("all-routes-glow")) {
      map.setLayoutProperty("all-routes-glow", "visibility", routesVisible ? "visible" : "none");
    }
  }, [mapMode]);
