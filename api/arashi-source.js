...
export default async function handler(req, res) {
  ...
  if (cache && Date.now() - cache.fetched_at < 15000) return res.status(200).json(cache);
  try {
    const response = await fetch(`${FEED}?targetSquareUid=${encodeURIComponent(UID)}&timeOffset=-1&filterType=ALL`, {
    ...
    if (raw.code !== '000000' || raw.success !== true || !Array.isArray(rows) || !rows.length || rows.length > 100) throw Error('schema');
    ...
    cache = {fetched_at: Date.now(), source: 'binance-square', author_id: UID, posts};
