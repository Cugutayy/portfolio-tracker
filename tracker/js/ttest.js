...
function renderTTestBody(){
  ...
  if(tTestTab==='basic'){
    ...
    // 5) Yöntemlerin tanımları — t-Test ve Sharpe Oranı
    h += cardCls('#5a4a8c', tr?'📐 t-Test ve Sharpe Oranı':'📐 t-Test and Sharpe Ratio',
      tr ? `<p style="margin:0 0 10px"><strong>t-Test</strong>, bir örneklem ortalamasının hipotez edilen bir değerden (genelde sıfır veya risksiz faiz) istatistiksel olarak anlamlı şekilde farklı olup olmadığını test eden parametrik bir yöntemdir. t-değeri, ortalamanın hipotez edilen değerden kaç standart hata uzakta olduğunu gösterir; kritik eşiği (t<sub>krit</sub>) aşarsa H₀ reddedilir ve gözlemlenen farkın tesadüfi olmadığı söylenebilir. Küçük örneklemlerde (n &lt; 30) normal dağılım yerine Student t-dağılımı kullanılır. Bu portföyde n=${nW} hafta ile çalışıldığı için test gücü düşüktür; sonuç bazen "yeterli istatistiksel kanıt yok" şeklinde çıkar, bu da dürüst bir bulgudur.</p>

      <p style="margin:0 0 10px"><strong>Sharpe Oranı</strong>, alınan birim risk başına elde edilen fazla getiriyi ölçer. Formülü: <strong>(Portföy getirisi − Risksiz faiz) ÷ Standart sapma</strong>. Pozitif değer, portföyün risksiz alternatifi (mevduat) geçtiğine işaret eder. Genel yorum: <em>1'in üzeri iyi, 2'nin üzeri mükemmel</em>. Aynı kazanç daha düşük dalgalanmayla elde edildiğinde Sharpe artar — yani metrik yalnız getiriye değil, alınan riske de bakar.</p>

      <p style="margin:0;padding:10px 12px;background:var(--surface);border-radius:6px;border-left:3px solid var(--accent)">İki metrik birbirini tamamlar: <strong>t-Test</strong> sonucun <strong>istatistiksel olarak anlamlı</strong> olup olmadığını söyler, <strong>Sharpe Oranı</strong> ise risk başına <strong>verimli</strong> olup olmadığını. Bir portföyün gerçek performansını değerlendirmek için her ikisine de bakılır.</p>`
      : '');
